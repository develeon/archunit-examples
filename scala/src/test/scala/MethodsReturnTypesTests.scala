...
object MethodsReturnTypesTests {
  ...
  ): DescribedPredicate[JavaMember] =
    new DescribedPredicate[JavaMember]("are in " + packageMatcher) {
      override def test(member: JavaMember): Boolean =
        packageMatcher.matches(member.getOwner.getPackage.getName)
    }

  def arePublic: DescribedPredicate[HasModifiers] =
    modifier(PUBLIC).as("are public")

  private def allMethods() =
    new AbstractClassesTransformer[JavaMethod]("methods") {
      override def doTransform(
        javaClasses: JavaClasses
      ): lang.Iterable[JavaMethod] = {
        javaClasses.asScala
  ...
  private def returnType(authorizedTypes: Class[_]*) =
    new ArchCondition[JavaMethod]("methods should return specified types") {
      override def check(method: JavaMethod, events: ConditionEvents): Unit = {
        val matches = authorizedTypes.exists(
          method.getRawReturnType.getFullName == _.getName
        )
        val message =
          s"${method.getFullName} return type ${method.getRawReturnType} is not authorized"
